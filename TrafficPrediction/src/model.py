...
class GCN(nn.Module):
    ...
    def forward(self, data, device=0):
...
class GraphAttentionLayer(nn.Module):
    ...
    def forward(self, features, adj, device):
        B, N = features.size(0), features.size(1)
        adj = adj + torch.eye(N, dtype=adj.dtype, device=device)  # A+I
...
class GAT(nn.Module):
    ...
    def forward(self, data, device=0):
        ...
        # forward
        outputs = self.act(torch.cat([attention(x, adj, device) for attention in self.attentions], dim=-1)).to(device)
        output_2 = self.act(self.conv2(outputs, adj, device))
