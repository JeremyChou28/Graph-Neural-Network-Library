...
class UnsupervisedLoss(object):
    ...
    def get_loss_sage(self, embeddings, nodes):
        ...
        assert False not in [nodes[i] == self.unique_nodes_batch[i] for i in
                             range(len(nodes))]  # 判断目标节点集和unique集里的节点是否1一一对应
...
class SageLayer(nn.Module):
    ...
    def __init__(self, input_size, out_size, gcn=False):
        ...
        self.weight = nn.Parameter(
            torch.FloatTensor(out_size, self.input_size if self.gcn else 2 * self.input_size))  # 初始化权重参数w*input.T
    ...
    def _get_unique_neighs_list(self, nodes, num_sample=10):
        ...
        if not num_sample is None:  # 如果num_sample为实数的话
            ...
            samp_neighs = [_set(_sample(to_neigh, num_sample)) if len(to_neigh) >= num_sample else to_neigh for to_neigh
                           in to_neighs]  # [set(随机采样的邻居集合),set(),set()]
...
if __name__ == "__main__":
    file_paths = {'cora_content': '../GCN/Cora_v2/cora.content', 'cora_cite': '../GCN/Cora_v2/cora.cites'}
    ...
    graphSage = GraphSage(2, feature_data.size(1), 128, feature_data, getattr(datacenter, ds + '_adj_lists'), device,
                          gcn='store_true', agg_func='MEAN')
    ...
    for epoch in range(epochs):
        print('----------------------EPOCH %d-----------------------' % epoch)
        graphSage, classification = apply_model(datacenter, ds, graphSage, classification, unsupervised_loss, 20,
                                                'normal', device, learn_method)
        if (epoch + 1) % 2 == 0 and learn_method == 'unsup':
            classification, max_vali_f1 = train_classification(datacenter, graphSage, classification, ds, device,
                                                               max_vali_f1, 'debug')
